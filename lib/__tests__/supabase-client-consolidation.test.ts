...
void describe('Bug Condition: Server-side singleton usage (Req 1.4)', () => {
  ...
  for (const file of serverSingletonFiles) {
    void it(`${file} should import from lib/supabase singleton, not createClient directly`, () => {
      ...
      assert.ok(hasSingletonImport, `${file} should import from @/lib/supabase`)
    })
  }

  void it('stripeService.ts should import from lib/supabase-admin', () => {
