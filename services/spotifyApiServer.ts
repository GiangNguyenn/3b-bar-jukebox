...
export async function getRelatedArtistsServer(
  ...
  try {
    ...
    console.log('[spotifyApiServer] Artist verified, fetching related artists via recommendations')
  } catch (verifyError) {
  ...
  try {
    console.log('[spotifyApiServer] Extracting related artists from top tracks (multi-level approach)')
    ...
    for (const track of seedTopTracks) {
      if (track.artists) {
        for (const artist of track.artists) {
          if (artist.id !== artistId && artist.id && artist.name && !artistMap.has(artist.id)) {
            const artistObj = {
              id: artist.id,
              name: artist.name
            }
    ...
    console.log('[spotifyApiServer] Level 1: Found', level1Artists.length, 'related artists from seed artist top tracks')
    ...
    if (relatedArtists.length < 10 && level1Artists.length > 0) {
      ...
      for (const level1Artist of artistsToProcess) {
        try {
          const level1TopTracks = await getArtistTopTracksServer(level1Artist.id, token)
          ...
          console.warn('[spotifyApiServer] Failed to get top tracks for level 1 artist:', {
      ...
      console.log('[spotifyApiServer] Level 2: Found', level2Artists.length, 'additional artists. Total:', relatedArtists.length)
    }
    ...
    for (const track of seedTopTracks) {
      if (track.artists && track.artists.length > 0) {
        ...
        if (primaryArtist.id !== artistId && relatedArtists.some(a => a.id === primaryArtist.id)) {
          if (!primaryArtists.some(a => a.id === primaryArtist.id)) {
            primaryArtists.push(relatedArtists.find(a => a.id === primaryArtist.id)!)
          }
        }
      }
    }
    ...
    console.log('[spotifyApiServer] Found', relatedArtists.length, 'related artists via top tracks (multi-level)', {
    ...
    console.error('[spotifyApiServer] Error extracting related artists from top tracks:', {
      artistId,
      error: error instanceof Error ? error.message : String(error)
    })
  }
  ...
  if (relatedArtists.length === 0) {
    try {
      console.log('[spotifyApiServer] Top tracks returned no collaborators, trying genre-based search')
      ...
      if (genres.length > 0) {
        ...
        console.log('[spotifyApiServer] Found', genreArtists.length, 'related artists via genre search', {
          genre: primaryGenre,
  ...
  if (relatedArtists.length === 0) {
    console.warn('[spotifyApiServer] All methods failed to find related artists, using popular artists fallback', {
      artistId
    })
    
    // Use a subset of popular artists, excluding the current artist
    const fallbackArtists = POPULAR_TARGET_ARTISTS
    ...
    console.log('[spotifyApiServer] Using', fallbackArtists.length, 'popular artists as fallback')
