...
// Mock state helper
function createMockState(paused: boolean, position: number, duration: number, trackId: string = 'track-1') {
    return {
        paused,
        position,
        duration,
...
test('BUG REPRO: isTrackFinished detects start-up buffering as track finish', async () => {
    ...
    assert.equal(isFinished, false, 'FIXED: Start-up buffering should NOT be detected as track finish')
})
