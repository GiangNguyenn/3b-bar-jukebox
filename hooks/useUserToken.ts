...
import { handleApiError, determineErrorType } from '@/shared/utils/errorHandling'
...
export function useUserToken() {
  ...
  const fetchTokenWithRetry = useCallback(async (attempt = 0): Promise<void> => {
    if (!username) {
    ...
    try {
      ...
      if (attempt > 0) {
        addLog('INFO', `Token recovery attempt ${attempt}/${TOKEN_RECOVERY_CONFIG.MAX_RETRY_ATTEMPTS}`, 'useUserToken')
      }

      const response = await fetch(`/api/token/${encodeURIComponent(username)}`)
      ...
      if (!response.ok) {
        ...
        if (errorType === ErrorType.AUTH && attempt < TOKEN_RECOVERY_CONFIG.MAX_RETRY_ATTEMPTS) {
          const delay = TOKEN_RECOVERY_CONFIG.RETRY_DELAYS[attempt] || TOKEN_RECOVERY_CONFIG.RETRY_DELAYS[TOKEN_RECOVERY_CONFIG.RETRY_DELAYS.length - 1]
          
          addLog('WARN', `Token error, retrying in ${delay}ms...`, 'useUserToken')
      ...
      if (attempt >= TOKEN_RECOVERY_CONFIG.MAX_RETRY_ATTEMPTS) {
        setConsecutiveFailures(prev => prev + 1)
        addLog('ERROR', 'Token recovery failed after max attempts', 'useUserToken')
      }
    } finally {
      setLoading(false)
  ...
  const isJukeboxOffline = consecutiveFailures >= TOKEN_RECOVERY_CONFIG.OFFLINE_THRESHOLD
