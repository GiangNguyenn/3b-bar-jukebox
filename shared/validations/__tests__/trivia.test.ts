...
test('trivia validations', async (t) => {
  await t.test(
    'triviaQuestionResponseSchema requires exactly 4 options',
    () => {
      // Valid response
      const valid = {
  ...
  await t.test(
    'triviaQuestionResponseSchema requires correctIndex between 0 and 3',
    () => {
      ...
      // Valid bounds
      assert.doesNotThrow(() =>
        triviaQuestionResponseSchema.parse(createWithIndex(0))
      )
      assert.doesNotThrow(() =>
        triviaQuestionResponseSchema.parse(createWithIndex(3))
      )

      // Invalid bounds
      assert.throws(() =>
        triviaQuestionResponseSchema.parse(createWithIndex(-1))
      )
      assert.throws(() =>
        triviaQuestionResponseSchema.parse(createWithIndex(4))
      )
    }
  )
})
