...
export function DgsDebugPanel({
    ...
    return (
      <div className='fixed bottom-4 right-4 z-50'>
        <button
          ...
          className='text-white flex items-center justify-center rounded-full bg-gray-800 p-3 shadow-lg transition-transform hover:scale-110 hover:bg-gray-700'
  ...
  return (
    <div className='fixed bottom-4 right-4 z-50 flex h-[600px] w-full max-w-lg flex-col overflow-hidden rounded-lg border border-gray-700 bg-gray-900 shadow-2xl'>
      ...
      <div className='flex items-center justify-between border-b border-gray-700 bg-gray-800 px-4 py-2'>
        ...
        <div className='flex items-center gap-2'>
          <button
            ...
            className='hover:text-white rounded p-1.5 text-gray-400 hover:bg-gray-700'
      ...
      {/* Content */}
      <div className='scrollbar-thin scrollbar-track-gray-900 scrollbar-thumb-gray-700 flex-1 overflow-y-auto p-4 text-xs'>
        {activeTab === 'overview' && (
          <OverviewTab
            roundTurn={roundTurn}
...
function OverviewTab({
  ...
  players,
  debugInfo
}: Omit<DgsDebugPanelProps, 'options'>): JSX.Element {
    ...
    <div className='space-y-4'>
      ...
      <Section title='Player Gravities'>
        <div className='space-y-2'>
          <GravityBar
            ...
            color='bg-blue-500'
            targetName={
              players?.find((p) => p.id === 'player1')?.targetArtist?.name
            }
          />
          <GravityBar
            label='Player 2'
            ...
            color='bg-green-500'
            targetName={
              players?.find((p) => p.id === 'player2')?.targetArtist?.name
            }
          />
        </div>
      </Section>
      ...
      <Section title='Exploration & System'>
        <div className='space-y-1 text-gray-400'>
          ...
          <div className='flex justify-between'>
            <span>Pool Size:</span>
            <span className='font-mono text-gray-200'>{candidatePoolSize}</span>
          </div>
          {hardConvergenceActive && (
            <div className='mt-2 rounded bg-red-900/30 p-2 text-center font-bold text-red-400'>
              HARD CONVERGENCE ACTIVE
            </div>
          )}
          {vicinity?.triggered && (
            <div className='mt-1 rounded bg-yellow-900/30 p-2 text-center font-bold text-yellow-400'>
              VICINITY TRIGGERED (
              {vicinity.playerId === 'player1' ? 'P1' : 'P2'})
            </div>
          )}
          ...
          {debugInfo?.targetProfiles && (
            <div className='mt-2 border-t border-gray-700 pt-2'>
              ...
              <div className='flex justify-between text-[10px]'>
                ...
                >
                  P1:{' '}
                  {debugInfo.targetProfiles.player1?.resolved ? 'OK' : 'FAIL'}
                </span>
                <span
                  className={
                ...
                >
                  P2:{' '}
                  {debugInfo.targetProfiles.player2?.resolved ? 'OK' : 'FAIL'}
                </span>
              </div>
            </div>
          )}
...
function PipelineTab({
  ...
  if (!debugInfo)
    return <div className='text-gray-500'>No debug info available</div>
  ...
  return (
    <div className='space-y-4'>
      ...
      {/* Stage 2: Selection */}
      <Section
        title={`Stage 2: Artist Scoring (Total ${debugInfo.scoring?.totalCandidates ?? 0})`}
      >
        {hasSelectedArtists ||
        (debugInfo.candidates && debugInfo.candidates.length > 0) ? (
          <div className='space-y-3'>
            <ArtistGroup
              category='CLOSER'
              artists={(debugInfo.candidates ?? selectedArtists).filter(
                (a) => a.category?.toUpperCase() === 'CLOSER'
              )}
            ...
            <ArtistGroup
              category='NEUTRAL'
              artists={(debugInfo.candidates ?? selectedArtists).filter(
                (a) => a.category?.toUpperCase() === 'NEUTRAL'
              )}
              color='text-yellow-400'
            />
            <ArtistGroup
              category='FURTHER'
              artists={(debugInfo.candidates ?? selectedArtists).filter(
                (a) => a.category?.toUpperCase() === 'FURTHER'
              )}
              color='text-red-400'
            />
          </div>
        ) : (
          <div className='italic text-gray-500'>No scored candidates data</div>
        )}
      </Section>
            ...
            return (
              ...
              >
                <div className='mb-1 flex items-center justify-between'>
                  <span className='w-2/3 truncate font-semibold text-gray-300'>
                    {opt.track?.name}
                  </span>
                  <span
                    className={`rounded px-1.5 py-0.5 text-[10px] font-bold ${
                      metrics.selectionCategory === 'closer'
                        ? 'bg-green-900 text-green-300'
                        : metrics.selectionCategory === 'further'
                ...
                <div className='mt-1 flex justify-between text-[10px] text-gray-500'>
                  <span>
                    Attraction:{' '}
                    {activePlayerId === 'player1'
                      ? metrics.aAttraction?.toFixed(3)
                      : metrics.bAttraction?.toFixed(3)}
                  </span>
...
function PerformanceTab({
  ...
  return (
    <div className='space-y-4'>
      ...
      <Section title='Execution Timing'>
        ...
        {timing && (
          <div className='space-y-1'>
            <TimingBar
              label='Pool'
              val={timing.candidatePoolMs}
              total={timing.totalMs}
              color='bg-blue-600'
            />
            <TimingBar
              label='Targets'
              val={timing.targetResolutionMs}
              total={timing.totalMs}
              color='bg-purple-600'
            />
            <TimingBar
              label='Enrich'
              val={timing.enrichmentMs}
              total={timing.totalMs}
              color='bg-yellow-600'
            />
            <TimingBar
              label='Scoring'
              val={timing.scoringMs}
              total={timing.totalMs}
              color='bg-green-600'
            />
            <TimingBar
              label='Select'
              val={timing.selectionMs}
              total={timing.totalMs}
              color='bg-red-600'
            />
          </div>
        ...
        {caching && (
          <div className='grid grid-cols-2 gap-2 text-[10px]'>
            <MetricBox
              ...
              value={`${((caching.cacheHitRate ?? 0) * 100).toFixed(1)}%`}
              color={
                (caching.cacheHitRate ?? 0) > 0.8
                  ? 'text-green-400'
                  : 'text-yellow-400'
              }
            />
            <MetricBox
              label='Spotify API Requests'
              value={caching.totalApiCalls || 0}
      ...
      {/* API Trace */}
      {perf?.apiCalls && perf.apiCalls.length > 0 && (
        <Section title={`API Trace (${perf.apiCalls.length} calls)`}>
          <div className='scrollbar-thin scrollbar-track-gray-800 scrollbar-thumb-gray-600 max-h-40 space-y-1 overflow-y-auto pr-1 text-[10px]'>
            {perf.apiCalls.map((call, i) => (
              <div
                key={i}
                className='flex justify-between border-b border-gray-700/50 pb-0.5 last:border-0'
              >
                <span className='truncate text-gray-400'>{call.operation}</span>
                <span
                  className={`font-mono ${
                    call.durationMs > 500
          ...
          <div className='space-y-1 text-[10px]'>
            <div className='flex justify-between'>
              ...
              <span className='font-mono text-blue-400'>
                {perf?.totalDbTimeMs ?? 0}ms
              </span>
            </div>
            <div className='flex justify-between'>
              <span>Queries:</span>
              <span className='font-mono text-gray-300'>
                {perf.dbQueries?.length ?? 0}
              </span>
            </div>
            {perf.slowestDbQuery && (
              <div className='mt-1 rounded bg-red-900/20 p-1'>
                <div className='font-semibold text-red-400'>Slowest Query:</div>
                <div className='truncate text-gray-500'>
                  {perf.slowestDbQuery.operation}
                </div>
                <div className='text-right font-mono text-red-300'>
                  {perf.slowestDbQuery.durationMs}ms
                </div>
              </div>
            )}
          </div>
...
function LogsTab({ debugInfo }: { debugInfo?: DgsDebugInfo }): JSX.Element {
  const logs = debugInfo?.pipelineLogs ?? []

  return (
    <div className='space-y-2 font-mono text-[10px]'>
      {logs.length === 0 && (
        <div className='italic text-gray-500'>No logs recorded</div>
      )}
      {logs.map((log, i) => (
...
function Section({
  ...
  return (
    <div className='mb-4 rounded-lg bg-gray-800/30 p-3'>
      <h3 className='mb-2 text-[10px] font-semibold uppercase tracking-wider text-gray-300'>
        {title}
      </h3>
      {children}
    </div>
...
function GravityBar({
  ...
  const normalize = (v: number): number => ((v - 0.15) / (0.7 - 0.15)) * 100
...
function ArtistGroup({
  ...
  return (
    <div>
      <div className={`mb-1 font-semibold ${color} text-[11px]`}>
        {category} ({artists.length})
      </div>
      <div className='ml-1 space-y-1 border-l border-gray-700 pl-2'>
        {artists.map((a, i) => (
          <div key={i} className='flex justify-between text-gray-400'>
            <span className='w-2/3 truncate'>{a.artistName}</span>
            <span className='font-mono text-gray-500'>
              {(a.attractionScore ?? a.simScore)?.toFixed(3)}
            </span>
          </div>
        ))}
      </div>
...
function TimingBar({
  ...
  return (
    <div className='flex items-center gap-2 text-[10px]'>
      <div className='w-16 shrink-0 text-right text-gray-400'>{label}</div>
      <div className='h-1.5 flex-1 overflow-hidden rounded-full bg-gray-700'>
        <div className={`h-full ${color}`} style={{ width: `${pct}%` }} />
      </div>
      <div className='w-12 shrink-0 text-right font-mono text-gray-300'>
        {val}ms
      </div>
    </div>
  )
