...
export async function forceRecovery(deviceId: string, fixedPlaylistId: string) {
  console.log('[Recovery] Starting force recovery process')
  try {
    if (!deviceId || !fixedPlaylistId) {
      throw new Error('Missing device ID or fixed playlist ID')
    }
    ...
    throw new Error(error instanceof Error ? error.message : 'Recovery failed')
  }
} 