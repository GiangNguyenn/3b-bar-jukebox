...
export function ConnectivityInvestigationSection({
    ...
    if (!investigation || (investigation.recentFailures.length === 0 && !investigation.lastInvestigation)) {
        return null
    }

    const { lastInvestigation, recentFailures, protocolStatus, suspectedIssues } = investigation
            ...
            <div className='space-y-4'>
                    ...
                    <div className='grid grid-cols-2 gap-3'>
                        ...
                        <div className='rounded-lg border border-gray-700 bg-gray-800/50 p-3'>
                            <div className='mb-1 flex items-center justify-between'>
                                ...
                                <span
                                    className={`text-xs font-semibold ${protocolStatus.ipv4.available
                                        ? 'text-green-400'
                        ...
                        <div className='rounded-lg border border-gray-700 bg-gray-800/50 p-3'>
                            <div className='mb-1 flex items-center justify-between'>
                                ...
                                <span
                                    className={`text-xs font-semibold ${protocolStatus.ipv6.available
                                        ? 'text-green-400'
                        ...
                        <div className='space-y-2 rounded-lg border border-gray-700 bg-gray-800/50 p-3 text-xs'>
                            ...
                            <div>
                                ...
                                <span className='font-mono text-gray-300'>
                                    {lastInvestigation.trigger.method} {lastInvestigation.trigger.url}
                                </span>
                            </div>
                            <div>
                                <span className='text-gray-400'>Error:</span>{' '}
                                <span className='text-red-400'>{lastInvestigation.trigger.error}</span>
                            </div>
                            ...
                            {lastInvestigation.recommendations.length > 0 && (
                                <div className='mt-2 border-t border-gray-700 pt-2'>
                                    ...
                                    <ul className='ml-3 list-disc space-y-0.5 text-gray-300'>
                                        {lastInvestigation.recommendations.map((rec: string, index: number) => (
                                            <li key={index}>{rec}</li>
                                        ))}
                                    </ul>
                                </div>
                ...
                {recentFailures.length > 0 && (
                    <div>
                        ...
                        <div className='space-y-1'>
                            {recentFailures.slice(0, 5).map((failure: FailedRequestInfo, index: number) => (
                                <div
                                    key={index}
