...
export default function AdminPage() {
  ...
  const trackResponseTime = async (operation: () => Promise<any>) => {
    ...
    try {
      ...
      setHealthStatus(prev => ({
        ...prev,
        connection: avgResponseTime < 500 ? 'good' : avgResponseTime < 1000 ? 'unstable' : 'poor'
      }))
    } catch (error) {
  ...
  useEffect(() => {
    const checkDeviceHealth = async () => {
      ...
      await trackResponseTime(async () => {
        ...
        setHealthStatus(prev => ({
          ...prev,
          device: 'healthy',
          playback: state.is_playing ? 'playing' : state.item ? 'paused' : 'stopped'
        }))
    ...
    deviceCheckInterval.current = setInterval(checkDeviceHealth, DEVICE_CHECK_INTERVAL)
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2 p-4 rounded-lg bg-gray-900/50 border border-gray-800">
            <div className={`w-3 h-3 rounded-full ${
              healthStatus.device === 'healthy' ? 'bg-green-500' :
              healthStatus.device === 'unresponsive' ? 'bg-yellow-500' :
              'bg-red-500'
            }`} />
            <span className="font-medium">
              {healthStatus.device === 'healthy' ? 'Device Connected' :
               healthStatus.device === 'unresponsive' ? 'Device Unresponsive' :
               'Device Disconnected'}
              {recoveryAttempts > 0 && ` (Recovery ${recoveryAttempts}/${MAX_RECOVERY_ATTEMPTS})`}
            </span>
          </div>

          <div className="flex items-center gap-2 p-4 rounded-lg bg-gray-900/50 border border-gray-800">
            <div className={`w-3 h-3 rounded-full ${
              healthStatus.playback === 'playing' ? 'bg-green-500' :
              healthStatus.playback === 'paused' ? 'bg-yellow-500' :
              healthStatus.playback === 'error' ? 'bg-red-500' :
              'bg-gray-500'
            }`} />
            <span className="font-medium">
              {healthStatus.playback === 'playing' ? 'Playback Active' :
               healthStatus.playback === 'paused' ? 'Playback Paused' :
               healthStatus.playback === 'error' ? 'Playback Error' :
               'Playback Stopped'}
            </span>
          </div>
          ...
          <div className="flex items-center gap-2 p-4 rounded-lg bg-gray-900/50 border border-gray-800">
            <div className={`w-3 h-3 rounded-full ${
              healthStatus.token === 'valid' ? 'bg-green-500' :
              healthStatus.token === 'expired' ? 'bg-yellow-500' :
              'bg-red-500'
            }`} />
            <span className="font-medium">
              {healthStatus.token === 'valid' ? 'Token Valid' :
               healthStatus.token === 'expired' ? 'Token Expiring' :
               'Token Error'}
            </span>
          </div>
          ...
          <div className="flex items-center gap-2 p-4 rounded-lg bg-gray-900/50 border border-gray-800">
            <div className={`w-3 h-3 rounded-full ${
              healthStatus.connection === 'good' ? 'bg-green-500' :
              healthStatus.connection === 'unstable' ? 'bg-yellow-500' :
              'bg-red-500'
            }`} />
            <span className="font-medium">
              {healthStatus.connection === 'good' ? 'Connection Good' :
               healthStatus.connection === 'unstable' ? 'Connection Unstable' :
               'Connection Poor'}
            </span>
          </div>
        </div>
