  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      <StatusIndicator
        ...
        label={healthStatus.device.charAt(0).toUpperCase() + healthStatus.device.slice(1)}
      />

      <StatusIndicator
        title="Playback State"
        status={playbackState?.is_playing ? 'playing' : playbackState?.item ? 'paused' : 'stopped'}
        colorMap={{
          playing: 'bg-green-500',
          paused: 'bg-yellow-500',
          stopped: 'bg-red-500'
        }}
        label={playbackState?.is_playing ? 'Playing' : playbackState?.item ? 'Paused' : 'Stopped'}
        subtitle={playbackState?.item?.name ? `- ${playbackState.item.name}` : undefined}
      />
      ...
      <StatusIndicator
        ...
        label={healthStatus.token.charAt(0).toUpperCase() + healthStatus.token.slice(1)}
        subtitle={healthStatus.tokenExpiringSoon ? '(Expiring Soon)' : undefined}
      />
