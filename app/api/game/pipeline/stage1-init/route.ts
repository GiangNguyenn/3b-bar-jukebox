...
} from '@/services/game/apiStatisticsTracker'
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const targetProfiles = await resolveTargetProfiles(safeTargets, token, statisticsTracker)
        ...
        if (!currentTrackId) {
            return NextResponse.json({ error: 'No current track found' }, { status: 400 })
        }

        // Fetch track details to get Artist ID reliably
        const { data: currentTrack } = await musicService.getTrack(currentTrackId, token)
        if (!currentTrack || !currentTrack.artists?.[0]?.id) {
            return NextResponse.json({ error: 'Invalid current track or missing artist' }, { status: 400 })
        }
