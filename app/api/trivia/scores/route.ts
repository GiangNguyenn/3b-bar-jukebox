...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const { data: existingScore, error: fetchError } = await supabaseAdmin
      ...
      .single() as { data: { score: number, first_score_at: string } | null, error: PostgrestError | null }
    ...
    const firstScoreAt = existingScore?.first_score_at ?? new Date().toISOString()
    ...
    if (upsertError) {
      ...
      return NextResponse.json({ error: 'Database error on upsert' }, { status: 500 })
    }
