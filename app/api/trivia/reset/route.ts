...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const { data: winnerData, error: rpcError } = await supabaseAdmin.rpc('trivia_determine_winner_and_reset', {
      p_profile_id: profile_id
    })

    if (rpcError) {
      logger('ERROR', 'Failed to execute reset RPC', rpcError.message)
      return NextResponse.json({ error: 'Database error on reset' }, { status: 500 })
    }
    ...
    if (winner) {
      const announcementText = 'Congratulations to ' + winner.winner_name + ' for winning the music trivia hour with ' + winner.winner_score + ' points!'
      ...
      if (announceError) {
        ...
        logger('ERROR', 'Failed to insert DJ announcement for winner', announceError.message)
      }
    }

    return NextResponse.json({
      winner: winner ? { player_name: winner.winner_name, score: winner.winner_score } : null,
      reset: true
    })
  } catch (err: unknown) {
