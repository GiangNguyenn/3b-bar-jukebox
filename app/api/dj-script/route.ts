...
export async function POST(request: NextRequest) {
  ...
  if (!trackName || typeof trackName !== 'string') {
    return NextResponse.json({ error: 'Missing required field: trackName' }, { status: 400 })
  }
  if (!artistName || typeof artistName !== 'string') {
    return NextResponse.json({ error: 'Missing required field: artistName' }, { status: 400 })
  }
  ...
  try {
    ...
    if (!script) {
      return NextResponse.json({ error: 'Venice AI returned an empty script' }, { status: 500 })
    }

    return NextResponse.json({ script })
  } catch {
    return NextResponse.json({ error: 'Failed to contact Venice AI' }, { status: 500 })
  }
}
