...
export async function POST() {
  try {
    ...
    const response = await fetch(`https://api.spotify.com/v1/users/${profile.spotify_user_id}/playlists`, {
      method: 'POST',
      headers: {
    ...
    if (insertError) {
      ...
      return NextResponse.json({ error: 'Failed to store playlist' }, { status: 500 });
    }
