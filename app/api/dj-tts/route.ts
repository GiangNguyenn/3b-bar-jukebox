...
export async function POST(request: NextRequest) {
  ...
  try {
    const veniceResponse = await fetch('https://api.venice.ai/api/v1/audio/speech', {
      method: 'POST',
      headers: {
    ...
    if (!veniceResponse.ok) {
      ...
      console.error('[dj-tts] Venice TTS error:', veniceResponse.status, errorBody)
    ...
    return NextResponse.json({ error: 'Failed to contact Venice AI TTS' }, { status: 500 })
  }
}
